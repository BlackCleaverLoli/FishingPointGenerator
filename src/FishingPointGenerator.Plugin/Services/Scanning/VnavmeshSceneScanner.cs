...
internal sealed class VnavmeshSceneScanner : ICurrentTerritoryScanner
{
    ...
    private const float CollisionPointIndexCellSize = 4f;
        ...
        CandidateGenerationDiagnostics? diagnostics)
    {
        ...
        var fishableBufferCells = BuildBufferedHorizontalCells(
            fishableTriangles,
            MaximumCandidateDistanceFromFishableSurface,
            SurfaceIndexCellSize);
        var fishableQueryCache = new FishableQueryCache(fishableIndex, diagnostics);
        diagnostics?.LogStep("build-fishable-buffer-cells", stepStopwatch, $"cells={fishableBufferCells.Count} buffer={MaximumCandidateDistanceFromFishableSurface:F1}");
        ...
        var walkableSurfaces = SelectPotentialWalkableSurfaces(walkableTriangles, fishableBufferCells, fishableQueryCache, diagnostics);
        diagnostics?.LogStep("filter-walkable-surfaces", stepStopwatch, $"input={walkableTriangles.Count} output={walkableSurfaces.Count} maxHorizontalDistance={MaximumCandidateDistanceFromFishableSurface:F1} maskCells={fishableBufferCells.Count}");
        ...
        stepStopwatch.Restart();
        var collisionBlockerLineIndex = new TriangleIndex(relevantCollisionBlockers, SurfaceIndexCellSize);
        diagnostics?.LogStep("build-collision-blocker-line-index", stepStopwatch, $"triangles={relevantCollisionBlockers.Count} cells={collisionBlockerLineIndex.CellCount} entries={collisionBlockerLineIndex.EntryCount} cellSize={SurfaceIndexCellSize:F1}");

        stepStopwatch.Restart();
        var collisionBlockerPointIndex = new TriangleIndex(relevantCollisionBlockers, CollisionPointIndexCellSize);
        diagnostics?.LogStep("build-collision-blocker-point-index", stepStopwatch, $"triangles={relevantCollisionBlockers.Count} cells={collisionBlockerPointIndex.CellCount} entries={collisionBlockerPointIndex.EntryCount} cellSize={CollisionPointIndexCellSize:F1}");
        ...
        AddRayDropWalkableCandidates(
            ...
            walkableSurfaces,
            walkableIndex,
            collisionBlockerLineIndex,
            collisionBlockerPointIndex,
            candidateDedupe,
            candidates,
            progress,
            cancellationToken,
    ...
    private static IReadOnlyList<ExtractedSceneTriangle> SelectPotentialWalkableSurfaces(
        IReadOnlyList<ExtractedSceneTriangle> walkableTriangles,
        IReadOnlySet<GridCell> fishableBufferCells,
        FishableQueryCache fishableQueryCache,
        CandidateGenerationDiagnostics? diagnostics)
    {
        var surfaces = new List<ExtractedSceneTriangle>();
        foreach (var triangle in walkableTriangles)
        {
            if (!TriangleOverlapsAnyCell(triangle, fishableBufferCells, SurfaceIndexCellSize))
            {
                if (diagnostics is not null)
                    diagnostics.WalkableSurfaceMaskRejects++;
                continue;
            }

            if (IsWalkableSurfaceNearFishableSurface(triangle, fishableQueryCache))
        ...
        FishableQueryCache fishableQueryCache)
    {
        ...
        {
            if (fishableQueryCache.TryFindNearest(
                    point,
                    MaximumCandidateDistanceFromFishableSurface,
                    out _,
                    out _,
    ...
    private static bool IsWalkableSampleOverlappingFishableSurface(
        ...
        FishableQueryCache fishableQueryCache)
    {
        foreach (var fishablePoint in fishableQueryCache.FindContainingPoints(walkablePoint))
        {
    ...
    private static IEnumerable<Vector3> EnumerateTriangleProximitySamples(
        ...
        float spacing)
    {
        return EnumerateTriangleProximitySamples(triangle, spacing, null, SurfaceIndexCellSize);
    }

    private static IEnumerable<Vector3> EnumerateTriangleProximitySamples(
        ExtractedSceneTriangle triangle,
        float spacing,
        IReadOnlySet<GridCell>? allowedCells,
        float allowedCellSize)
    {
        var keys = new HashSet<ProbePointKey>();
        foreach (var point in EnumerateTriangleFeatureSamples(triangle, spacing))
        {
            if (allowedCells is not null && !allowedCells.Contains(GridCell.From(point.X, point.Z, allowedCellSize)))
                continue;
        ...
        var samplePoints = allowedCells is null
            ? EnumerateTriangleSamples(triangle, spacing)
            : EnumerateTriangleSamples(triangle, spacing, allowedCells, allowedCellSize);
    ...
    private static bool TryGetNearestBlockCenterFacingAngle(
        Vector3 position,
        ...
        out float angle)
    {
        ...
        {
            if (!TryGetHorizontalDirection(block.Center - position, out var direction))
                continue;

            var distanceSquared = HorizontalDistanceSquared(position, block.Center);
    ...
    private static void AddRayDropWalkableCandidates(
        ...
        TriangleIndex walkableIndex,
        TriangleIndex collisionBlockerLineIndex,
        TriangleIndex collisionBlockerPointIndex,
        CandidateDedupeIndex candidateDedupe,
        List<CandidateScratch> candidates,
        IProgress<TerritoryScanProgress>? progress,
        CancellationToken cancellationToken,
        CandidateGenerationDiagnostics? diagnostics)
    {
        ...
        var queryCache = new ScanQueryCache(walkableIndex, collisionBlockerLineIndex, collisionBlockerPointIndex, diagnostics);
        var rayDropTargetCache = new RayDropTargetCache(territoryId, fishableIndex, fishableSurfaceGroupIds, queryCache, diagnostics);
        ...
        for (var surfaceIndex = 0; surfaceIndex < walkableSurfaces.Count; surfaceIndex++)
        {
            ...
            foreach (var position in EnumerateTriangleProximitySamples(
                         walkableSurfaces[surfaceIndex],
                         WalkableBlockCandidateSampleSpacing,
                         fishableBufferCells,
                         SurfaceIndexCellSize))
            {
                cancellationToken.ThrowIfCancellationRequested();
                ...
                if (diagnostics is not null)
                    diagnostics.WalkableSampleCandidates++;
                if (!sampledPoints.Add(WalkableCandidateStartKey.From(position)))
                    continue;

                if (candidateDedupe.ContainsNear(position))
                {
                    if (diagnostics is not null)
                        diagnostics.CandidatePreDedupeRejects++;
                    continue;
                }

                if (!fishableBufferCells.Contains(GridCell.From(position.X, position.Z, SurfaceIndexCellSize)))
                {
                    if (diagnostics is not null)
                        diagnostics.WalkableSampleMaskRejects++;
                    continue;
                }
                ...
                if (!fishableQueryCache.TryFindNearest(
                        ...
                        out _))
                    continue;
                var preferredFacingAngle = TryGetHorizontalDirection(nearestFishablePoint - position, out var preferredDirection)
                    ? DirectionToAngle(preferredDirection)
                    : (float?)null;

                if (IsWalkableSampleOverlappingFishableSurface(position, fishableQueryCache))
                {
                    if (diagnostics is not null)
    ...
    private static void AddCoverageDrivenCandidates(
        ...
        CandidateGenerationDiagnostics? diagnostics)
    {
        var queryCache = new ScanQueryCache(walkableIndex, collisionBlockerIndex, collisionBlockerIndex, diagnostics);
        ...
        CandidateGenerationDiagnostics? diagnostics)
    {
        ...
        foreach (var walkableSurface in walkableSurfaces)
        {
            ...
            foreach (var position in EnumerateTriangleSamples(walkableSurface, WalkableBlockCandidateSampleSpacing))
            {
                ...
                if (diagnostics is not null)
                    diagnostics.CandidateCreateAttempts++;
                var preferredFacingAngle = TryGetNearestBlockCenterFacingAngle(position, nearbyBlocks, out var blockCenterAngle)
                    ? blockCenterAngle
                    : (float?)null;
                if (!TryCreateRayDropCandidateForWalkableSample(
                        position,
    ...
    private static bool TryCreateRayDropCandidateForWalkableSample(
        RayDropTargetCache rayDropTargetCache,
        Vector3 walkablePoint,
        float? preferredFacingAngle,
        ScanQueryCache queryCache,
        ...
        out CandidateCreateFailureReason failureReason)
    {
        ...
        {
            queryCache.RecordRayDropProbeAttempt();
            ...
            var result = rayDropTargetCache.TryFind(rayPoint, out var target);
        ...
        out DirectionalRayDropSector? selectedSector)
    {
        ...
        if (preferredFacingAngle is { } preferredAngle)
        {
            var preferredHits = new List<DirectionalRayDropHit>(8);
            var preferredTestedAngles = new HashSet<int>();
            ProbeAngles(probe, preferredHits, preferredTestedAngles, GetPreferredRayDropProbeAngles(preferredAngle));
            if (preferredHits.Count > 0
                && TryResolveBestRayDropSector(probe, isUsable, preferredHits, preferredTestedAngles, out selected, out selectedSector))
                return true;
        }
    ...
    private static IReadOnlyList<float> GetPreferredRayDropProbeAngles(float preferredAngle) =>
    [
        preferredAngle,
        preferredAngle + MathF.PI / 12f,
        preferredAngle - MathF.PI / 12f,
        preferredAngle + MathF.PI / 6f,
        preferredAngle - MathF.PI / 6f,
    ];
    ...
    private static IEnumerable<Vector3> EnumerateTriangleSamples(
        ExtractedSceneTriangle triangle,
        float spacing,
        IReadOnlySet<GridCell> allowedCells,
        float allowedCellSize)
    {
        ...
        var minZ = MathF.Min(triangle.A.Z, MathF.Min(triangle.B.Z, triangle.C.Z));
        var maxZ = MathF.Max(triangle.A.Z, MathF.Max(triangle.B.Z, triangle.C.Z));
        var minCell = GridCell.From(minX, minZ, allowedCellSize);
        var maxCell = GridCell.From(maxX, maxZ, allowedCellSize);

        for (var cellX = minCell.X; cellX <= maxCell.X; cellX++)
        {
            for (var cellZ = minCell.Z; cellZ <= maxCell.Z; cellZ++)
            {
                var cell = new GridCell(cellX, cellZ);
                if (!allowedCells.Contains(cell))
                    continue;

                var cellMinX = MathF.Max(minX, cellX * allowedCellSize);
                var cellMaxX = MathF.Min(maxX, (cellX + 1) * allowedCellSize);
                var cellMinZ = MathF.Max(minZ, cellZ * allowedCellSize);
                var cellMaxZ = MathF.Min(maxZ, (cellZ + 1) * allowedCellSize);
                var firstX = MathF.Ceiling(cellMinX / spacing) * spacing;
                var firstZ = MathF.Ceiling(cellMinZ / spacing) * spacing;
                for (var x = firstX; x <= cellMaxX; x += spacing)
                {
                    for (var z = firstZ; z <= cellMaxZ; z += spacing)
                    {
                        if (TryProjectYOnTriangleXz(triangle, x, z, out var y))
                            yield return new Vector3(x, y, z);
                    }
        ...
        IReadOnlyList<ExtractedSceneTriangle> collisionBlockerTriangles)
    {
        ...
        var fishableBufferCells = BuildBufferedHorizontalCells(
            ...
            MaximumCandidateDistanceFromFishableSurface,
            SurfaceIndexCellSize);
        var fishableQueryCache = new FishableQueryCache(fishableIndex, null);
        var walkableSurfaces = SelectPotentialWalkableSurfaces(walkableTriangles, fishableBufferCells, fishableQueryCache, null);
        if (walkableSurfaces.Count == 0)
        {
            pluginLog.Debug(
    ...
    private sealed class CandidateGenerationDiagnostics
    {
        ...
        public long CandidatePreDedupeRejects { get; set; }
        public long WalkableSurfaceMaskRejects { get; set; }
        public long WalkableSampleCandidates { get; set; }
        public long WalkableSampleMaskRejects { get; set; }
        public long FishableNearQueries { get; set; }
        public long FishableNearCacheHits { get; set; }
        public long FishableNearHits { get; set; }
        public long FishableNearMisses { get; set; }
        public long RayDropProbeAttempts { get; set; }
        public long RayDropTargetQueries { get; set; }
        public long RayDropTargetCacheHits { get; set; }
        public long CollisionBelowQueries { get; set; }
        public long CollisionBelowCacheHits { get; set; }
        ...
        public void LogSummary(Stopwatch stopwatch, int candidateCount)
        {
            ...
            log.Information(
                ...
                CandidatePreDedupeRejects,
                WalkableSurfaceMaskRejects,
                WalkableSampleCandidates,
                WalkableSampleMaskRejects,
                FishableNearQueries,
                FishableNearCacheHits,
                FishableNearHits,
                FishableNearMisses,
                RayDropProbeAttempts,
                RayDropTargetQueries,
                RayDropTargetCacheHits,
                CollisionBelowQueries,
                CollisionBelowCacheHits,
                ProbeAttempts,
    ...
    private sealed class RayDropTargetCache
    {
        private readonly uint territoryId;
        private readonly TriangleIndex fishableIndex;
        private readonly IReadOnlyDictionary<ExtractedSceneTriangle, string> fishableSurfaceGroupIds;
        private readonly ScanQueryCache queryCache;
        private readonly CandidateGenerationDiagnostics? diagnostics;
        private readonly Dictionary<ExactProbePointKey, CachedRayDropTargetResult> targets = [];

        public RayDropTargetCache(
            uint territoryId,
            TriangleIndex fishableIndex,
            IReadOnlyDictionary<ExtractedSceneTriangle, string> fishableSurfaceGroupIds,
            ScanQueryCache queryCache,
            CandidateGenerationDiagnostics? diagnostics)
        {
            ...
            this.fishableSurfaceGroupIds = fishableSurfaceGroupIds;
            this.queryCache = queryCache;
            this.diagnostics = diagnostics;
        }

        public RayDropProbeResult TryFind(Vector3 rayPoint, out RayDropFishableTarget target)
        {
            if (diagnostics is not null)
                diagnostics.RayDropTargetQueries++;

            var key = ExactProbePointKey.From(rayPoint);
            if (targets.TryGetValue(key, out var cached))
            {
                if (diagnostics is not null)
                    diagnostics.RayDropTargetCacheHits++;
            ...
            var result = TryFindRayDropFishableTarget(
                ...
                queryCache,
                out target);
            targets[key] = new CachedRayDropTargetResult(result, target);
            return result;
        }
    }

    private sealed class FishableQueryCache
    {
        private readonly TriangleIndex index;
        private readonly CandidateGenerationDiagnostics? diagnostics;
        private readonly Dictionary<FishableNearestQueryKey, CachedNearestTriangleResult> nearest = [];
        private readonly Dictionary<ExactHorizontalProbeKey, IReadOnlyList<Vector3>> containingPoints = [];

        public FishableQueryCache(TriangleIndex index, CandidateGenerationDiagnostics? diagnostics)
        {
            this.index = index;
            this.diagnostics = diagnostics;
        }
        ...
        {
            if (diagnostics is not null)
                diagnostics.FishableNearQueries++;

            var key = FishableNearestQueryKey.From(point, radius);
            if (nearest.TryGetValue(key, out var cached))
            {
                if (diagnostics is not null)
                    diagnostics.FishableNearCacheHits++;
                return AssignNearestResult(cached, out triangle, out nearestPoint, out distance);
            }

            var found = index.TryFindNearest(point, radius, out triangle, out nearestPoint, out distance);
            cached = new CachedNearestTriangleResult(found, triangle, nearestPoint, distance);
            nearest[key] = cached;
            return RecordAndAssignNearestResult(cached, out triangle, out nearestPoint, out distance);
        }

        public IReadOnlyList<Vector3> FindContainingPoints(Vector3 point)
        {
            var key = ExactHorizontalProbeKey.From(point);
            if (containingPoints.TryGetValue(key, out var points))
                return points;

            points = index.FindContainingPoints(point);
            containingPoints[key] = points;
            return points;
        }

        private bool AssignNearestResult(
            CachedNearestTriangleResult cached,
            out ExtractedSceneTriangle triangle,
            out Vector3 nearestPoint,
            out float distance)
        {
            RecordNearestResult(cached.Found);
            triangle = cached.Found ? cached.Triangle : default;
            nearestPoint = cached.Found ? cached.Point : default;
            distance = cached.Found ? cached.Distance : float.MaxValue;
            return cached.Found;
        }

        private bool RecordAndAssignNearestResult(
            CachedNearestTriangleResult cached,
            out ExtractedSceneTriangle triangle,
            out Vector3 nearestPoint,
            out float distance)
        {
            ...
            nearestPoint = cached.Point;
            distance = cached.Distance;
            return cached.Found;
        }

        private void RecordNearestResult(bool found)
        {
            if (diagnostics is null)
                return;

            if (found)
                diagnostics.FishableNearHits++;
            else
                diagnostics.FishableNearMisses++;
        }
    }
    ...
    private sealed class ScanQueryCache
    {
        ...
        private readonly TriangleIndex collisionBlockerLineIndex;
        private readonly TriangleIndex collisionBlockerPointIndex;
        private readonly CandidateGenerationDiagnostics? diagnostics;
        ...
        private readonly Dictionary<ExactProbePointKey, CachedTrianglePointResult> highestCollisionBelow = [];
            ...
            CandidateGenerationDiagnostics? diagnostics)
        {
            ...
            this.collisionBlockerLineIndex = collisionBlockerLineIndex;
            this.collisionBlockerPointIndex = collisionBlockerPointIndex;
            this.diagnostics = diagnostics;
        }
        ...
        public bool HasStandingClearance(Vector3 walkablePoint)
        {
            ...
            if (!standingClearance.TryGetValue(key, out var result))
            {
                ...
                result = !collisionBlockerPointIndex.HasContainingPointAboveWithinVerticalRange(
                    walkablePoint,
                    WalkableFishableMinimumVerticalDelta,
                    WalkableStandingClearanceMeters);
        ...
        {
            if (diagnostics is not null)
                diagnostics.CollisionBelowQueries++;

            var key = ExactProbePointKey.From(rayPoint);
            if (highestCollisionBelow.TryGetValue(key, out var cached))
            {
                if (diagnostics is not null)
                    diagnostics.CollisionBelowCacheHits++;
                collisionTriangle = cached.Found ? cached.Triangle : default;
                collisionPoint = cached.Found ? cached.Point : default;
                return cached.Found;
            }

            var found = collisionBlockerPointIndex.TryFindHighestContainingPointBelow(
                rayPoint,
                WalkableFishableMinimumVerticalDelta,
                out collisionTriangle,
                out collisionPoint);
            highestCollisionBelow[key] = new CachedTrianglePointResult(found, collisionTriangle, collisionPoint);
            return found;
        }
        ...
        public void RecordRayDropProbeAttempt()
        {
            if (diagnostics is not null)
                diagnostics.RayDropProbeAttempts++;
        }
        ...
        public bool HasClearRayDropHorizontalLine(Vector3 walkablePoint, Vector3 rayPoint)
        {
            ...
            {
                result = !collisionBlockerLineIndex.IntersectsSegment(start, rayPoint);
        ...
        public bool HasClearCandidateRearLine(Vector3 candidatePosition, float rotation)
        {
            ...
            {
                result = !collisionBlockerLineIndex.IntersectsSegment(start, end);
        ...
        private bool HasCollisionBlockerAboveFacingCorridor(Vector3 walkablePoint, Vector3 fishablePoint)
        {
                 ...
                 offset += FacingCorridorProbeStep)
            {
                ...
                if (collisionBlockerPointIndex.HasContainingPointAboveWithinVerticalRange(
                        probe,
    ...
    private sealed class TriangleIndex
    {
        ...
        private readonly Dictionary<GridCell, List<IndexedTriangle>> cells = [];

        public TriangleIndex(IReadOnlyList<ExtractedSceneTriangle> triangles, float cellSize)
        {
            this.cellSize = cellSize;
            foreach (var triangle in triangles)
                Add(new IndexedTriangle(triangle));
        }
        ...
        public IReadOnlyList<Vector3> FindContainingPoints(Vector3 point)
        {
            ...
            {
                if (!candidate.ContainsXz(point2)
                    || !candidate.TryProjectY(point.X, point.Z, out var candidateY)
        ...
        public bool IntersectsSegment(Vector3 start, Vector3 end)
        {
            ...
            for (var x = minCell.X; x <= maxCell.X; x++)
            {
                for (var z = minCell.Z; z <= maxCell.Z; z++)
                {
                    ...
                    foreach (var candidate in triangles)
                    {
                        if (candidate.SegmentAabbMayIntersect(start, end)
                            && SegmentIntersectsTriangle(start, end, candidate.Source))
                            return true;
                    }
                }
            }
        ...
        public bool TryFindNearest(
            ...
            out float distance)
        {
            var minCell = GridCell.From(point.X - radius, point.Z - radius, cellSize);
            var maxCell = GridCell.From(point.X + radius, point.Z + radius, cellSize);
            ...
            for (var x = minCell.X; x <= maxCell.X; x++)
            {
                for (var z = minCell.Z; z <= maxCell.Z; z++)
                {
                    ...
                    foreach (var candidate in triangles)
                    {
                        var candidateDistanceSquared = candidate.ClosestHorizontalDistanceSquared(point, out var candidatePoint);
                        if (candidateDistanceSquared > radiusSquared
                            || candidateDistanceSquared >= bestDistanceSquared)
                            continue;

                        if (predicate is not null && !predicate(candidate.Source, candidatePoint))
                            continue;
        ...
        private void Add(IndexedTriangle triangle)
        {
            var minCell = GridCell.From(triangle.MinX, triangle.MinZ, cellSize);
            var maxCell = GridCell.From(triangle.MaxX, triangle.MaxZ, cellSize);
        ...
        private sealed class IndexedTriangle
        {
            private readonly Vector2 a2;
            private readonly Vector2 b2;
            private readonly Vector2 c2;
            private readonly float v0X;
            private readonly float v0Z;
            private readonly float v1X;
            private readonly float v1Z;
            private readonly float denominator;
            private readonly float fallbackY;

            public IndexedTriangle(ExtractedSceneTriangle source)
            {
                Source = source;
                a2 = new Vector2(source.A.X, source.A.Z);
                b2 = new Vector2(source.B.X, source.B.Z);
                c2 = new Vector2(source.C.X, source.C.Z);
                v0X = source.B.X - source.A.X;
                v0Z = source.B.Z - source.A.Z;
                v1X = source.C.X - source.A.X;
                v1Z = source.C.Z - source.A.Z;
                denominator = (v0X * v1Z) - (v1X * v0Z);
                fallbackY = source.Centroid.Y;
                MinX = MathF.Min(source.A.X, MathF.Min(source.B.X, source.C.X));
                MaxX = MathF.Max(source.A.X, MathF.Max(source.B.X, source.C.X));
                MinY = TriangleMinY(source);
                MaxY = TriangleMaxY(source);
                MinZ = MathF.Min(source.A.Z, MathF.Min(source.B.Z, source.C.Z));
                MaxZ = MathF.Max(source.A.Z, MathF.Max(source.B.Z, source.C.Z));
            }

            public ExtractedSceneTriangle Source { get; }
            ...
            public bool ContainsXz(Vector2 point) => PointInTriangle(point, a2, b2, c2);

            public bool TryProjectY(float x, float z, out float y)
            {
                if (MathF.Abs(denominator) <= 0.0001f)
                {
                    y = 0f;
                ...
                var v2X = x - Source.A.X;
                var v2Z = z - Source.A.Z;
                var bWeight = ((v2X * v1Z) - (v1X * v2Z)) / denominator;
                var cWeight = ((v0X * v2Z) - (v2X * v0Z)) / denominator;
                var aWeight = 1f - bWeight - cWeight;
                if (aWeight < -0.001f || bWeight < -0.001f || cWeight < -0.001f)
                ...
                y = (Source.A.Y * aWeight) + (Source.B.Y * bWeight) + (Source.C.Y * cWeight);
                return true;
            }

            public float ClosestHorizontalDistanceSquared(Vector3 point, out Vector3 closestPoint)
            {
                var point2 = new Vector2(point.X, point.Z);
                var closest = ContainsXz(point2)
                    ? point2
                    : ClosestPointOnTriangleEdge(point2, a2, b2, c2);

                closestPoint = new Vector3(closest.X, fallbackY, closest.Y);
                if (TryProjectY(closestPoint.X, closestPoint.Z, out var y))
                    closestPoint.Y = y;

                var dx = point.X - closestPoint.X;
                var dz = point.Z - closestPoint.Z;
                return (dx * dx) + (dz * dz);
            }

            public bool SegmentAabbMayIntersect(Vector3 start, Vector3 end)
            {
                var segmentMinX = MathF.Min(start.X, end.X) - SightLineIntersectionEpsilon;
                var segmentMaxX = MathF.Max(start.X, end.X) + SightLineIntersectionEpsilon;
                var segmentMinY = MathF.Min(start.Y, end.Y) - SightLineIntersectionEpsilon;
    ...
    private readonly record struct ExactHorizontalProbeKey(int X, int Z)
    {
        public static ExactHorizontalProbeKey From(Vector3 point) => new(
            point.X == 0f ? 0 : BitConverter.SingleToInt32Bits(point.X),
            point.Z == 0f ? 0 : BitConverter.SingleToInt32Bits(point.Z));
    }

    private readonly record struct ExactProbePointKey(int X, int Y, int Z)
    {
        public static ExactProbePointKey From(Vector3 point) => new(
            point.X == 0f ? 0 : BitConverter.SingleToInt32Bits(point.X),
            point.Y == 0f ? 0 : BitConverter.SingleToInt32Bits(point.Y),
            point.Z == 0f ? 0 : BitConverter.SingleToInt32Bits(point.Z));
    }

    private readonly record struct FishableNearestQueryKey(ExactHorizontalProbeKey Point, int Radius)
    {
        public static FishableNearestQueryKey From(Vector3 point, float radius) => new(
            ExactHorizontalProbeKey.From(point),
            BitConverter.SingleToInt32Bits(radius));
    }
    ...
    private readonly record struct CachedTrianglePointResult(
        bool Found,
        ExtractedSceneTriangle Triangle,
        Vector3 Point);

    private readonly record struct CachedNearestTriangleResult(
        bool Found,
        ExtractedSceneTriangle Triangle,
        Vector3 Point,
        float Distance);

    private readonly record struct CachedRayDropTargetResult(
        RayDropProbeResult Result,
        RayDropFishableTarget Target);
