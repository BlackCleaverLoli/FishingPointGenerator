...
用途：旧版 spot 级候选点缓存，仅作为兼容回退。当前主流程使用 `data/generated/territory_{territoryId}.json` 的 Territory 全图缓存按需派生推荐候选，不再把 spot scan 文件作为长期维护数据。
...
从全图缓存派生到单个钓场时，候选会记录 `distanceToTargetCenterMeters` 与 `isWithinTargetSearchRadius`。推荐和状态统计只看推荐范围内候选；抛竿自动点亮仍可使用全图候选作为实证兜底。
