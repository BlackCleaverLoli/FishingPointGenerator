...
spot-driven 主流程在插件层用 `SpotScanService` 包装该接口：底层仍扫描当前 territory 几何，再按当前 `FishingSpotTarget` 过滤并保存为 `data/scans/territory_{territoryId}/spot_{fishingSpotId}.scan.json`。
